...
print check(flux)
fixed_flux = nan_to_median(flux)
print check(fixed_flux)

# for i, e in enumerate(flux):
# 	flux[i] = ((e - median) / median)

plt.plot(fixed_flux, color = 'Red', marker = ',', linestyle = 'None')

plt.plot(marginalize(fixed_flux, 100), color = 'Blue', marker = ',', linestyle = 'None')
